...
Tone is not free, but opacity outweighs it about three to one -- and once the file is
flattened the tone difference vanishes entirely, because the background is then doing
most of the damage. The mechanism is area: an alpha channel damages only the modules
the ink covers, a flat file damages the whole bounding rectangle. So the sparser the
mark the worse it is, which is backwards from how the files arrive -- line art is the
sparsest and the likeliest to be shipped flat.

That matters because it is invisible in the preview: a flattened white background
looks like nothing at all against a white canvas, while quietly holding the safe logo
size down. The design just gets a smaller `estimatedSafeScale` and the user is given
no reason why.
